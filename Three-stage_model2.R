##############################################################################
# R script for the Three-stage model 2
# 
        ...
        for(n in 1:6){  # variations in the number of phases
          ...
          for(p in 1:6){  # variations in the proportion of the breeding season
            ...
            for(i in 1:n.season){
              ...
              elast.4rates[g,wj,wa,wg1,n,p,,i] <- c(sum(elast.i[,1]), 
                                                    sum(elast.i * 
...
thres.null.sj <- function(x){
  cx <- 1 / (1 + exp(h*x))
  mpm.exp <- matrix(c(s1*cx*(1-g1), f2,        f3,
                      s1*cx*g1,     s2*(1-g2), 0,
                      0,            s2*g2,     s3), 3, byrow = T)
...
thres.sj <- function(x){
  cx <- 1 / (1 + exp(h*x))
  mpm.r.exp <- matrix(c(s1*cx*(1-g1), f2,        f3,
                        s1*cx*g1,     s2*(1-g2), 0,
                        0,            s2*g2,     s3), 3, byrow = T)
...
thres.null.sa <- function(x){
  cx <- 1 / (1 + exp(h*x))
  mpm.exp <- matrix(c(s1*(1-g1), f2,           f3,
                      s1*g1,     s2*cx*(1-g2), 0,
                      0,         s2*cx*g2,     s3*cx), 3, byrow = T)
  mpm.expy <- mpm.exp %^% n.season
  lambda.expy <- Re(eigen(mpm.expy)$values[1])
  abs(log(lambda.expy))
}
...
thres.sa <- function(x){
  cx <- 1 / (1 + exp(h*x))
  mpm.r.exp <- matrix(c(s1*(1-g1), f2,           f3,
                        s1*g1,     s2*cx*(1-g2), 0,
                        0,         s2*cx*g2,     s3*cx), 3, byrow = T)
  mpm.w.exp <- matrix(c(s1*w1*(1-g1*wg), 0,                  0,
                        s1*w1*g1*wg,     s2*w2*cx*(1-g2*wg), 0,
                        0,               s2*w2*cx*g2*wg,     s3*w3*cx),
                      3, byrow = T)
  mpm.exp <- (mpm.w.exp %^% (n.season-n.replength)) %*% (mpm.r.exp %^% n.replength)
  lambda.exp <- Re(eigen(mpm.exp)$values[1])
  abs(log(lambda.exp))
...
thres.null.f <- function(x){
  cx <- 1 / (1 + exp(h*x))
  mpm.exp <- matrix(c(s1*(1-g1), f2*cx,     f3*cx,
                      s1*g1,     s2*(1-g2), 0,
                      0,         s2*g2,     s3), 3, byrow = T)
  mpm.expy <- mpm.exp %^% n.season
  lambda.expy <- Re(eigen(mpm.expy)$values[1])
...
thres.f <- function(x){
  cx <- 1 / (1 + exp(h*x))
  mpm.r.exp <- matrix(c(s1*(1-g1), f2*cx,         f3*cx,
                        s1*g1,     s2*(1-g2), 0,
                        0,         s2*g2,     s3), 3, byrow = T)
  mpm.w.exp <- matrix(c(s1*w1*(1-g1*wg), 0,                 0,
                        s1*w1*g1*wg,     s2*w2*(1-g2*wg), 0,
...
thres.null.g <- function(x){
  cx <- 1 / (1 + exp(h*x))
  mpm.exp <- matrix(c(s1*(1-g1*cx), f2,           f3,
...
thres.g <- function(x){
  cx <- 1 / (1 + exp(h*x))
  mpm.r.exp <- matrix(c(s1*(1-g1*cx), f2,           f3,
                        s1*g1*cx,     s2*(1-g2*cx), 0,
                        0,            s2*g2*cx,     s3), 3, byrow = T)
...
ec95.sj <- function(x){
  ...
  mpm.r.exp <- matrix(c(s1*cx*(1-g1), f2,        f3,
                        ...
                        0,            s2*g2,     s3), 3, byrow = T)
  mpm.w.exp <- matrix(c(s1*w1*cx*(1-g1*wg), 0,               0,
                        s1*w1*cx*g1*wg,     s2*w2*(1-g2*wg), 0,
                        0,                  s2*w2*g2*wg,     s3*w3),
                      3, byrow = T)
  mpm.exp <- (mpm.w.exp %^% (n.season-n.replength)) %*% (mpm.r.exp %^% n.replength)
  lambda.exp <- Re(eigen(mpm.exp)$values[1])
...
save.image("Three-stage_model2.RData")
...
png("Three-stage2_Fig2.png", width = 3240, height = 3240, res = 300)
...
png("Three-stage2_Fig3.png", width = 5040, height = 3240, res = 300)
...
axis(2, 0:10*4000, 0:10*4000, cex.axis=3, las=1)
par(mgp=c(3,1.5,0))
...
#   Fig S12
# ----------------------------------------
png("Three-stage2_Fig4.png", width = 4000, height = 3200, res = 300)
...
png("Three-stage2_Fig5.png", width = 4200, height = 6000, res = 300)
